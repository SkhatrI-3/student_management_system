...
<body class="antialiased" >
    ...
    <div class=" hero container-fluid" data-aos="fade-up">
        <h1>SEE YOUR GIVEN ASSIGNMENT</h1>
        <h2>HERE YOU CAN MANAGE YOUR STUDENTS DIGITALLY</h2>
...
<h2 class="breadcrumb-item text-center mt-1"><a class="text-center" href="{{route('teacher.home')}}">GO TO DASHBOARD  </a></h2>
    ...
    <div class="container">
        ...
        <div class="add w-100 text-center d-flex justify-content-center align-item-center ">
            ...
            <a class="text-center text-decoration-none m-3 teacher fs-4 w-100 text-secondary" href="{{route('assignments.create')}} ">Add assignment</a>
        </div>
        <table class="table table-secondary table-striped table-hover table-bordered table-sm table-responsive-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><img src="{{ asset('uploads/' . $assignment->file) }}" alt="{{ $assignment->name }}" width="100" height="100"></td>
                    <td>{{$assignment->deadline}}</td>
                    <td>
                        {{-- @php
                        $attendance = \App\Models\Attendance::where('student_id', '=', $student->id)->where('date', '=', date_format(now(), 'Y-m-d'))->first();
                        @endphp
                        
                        @if ($attendance)
                            @if ($attendance->status == 'Absent')
                                <span class="badge bg-danger p-2">{{ $attendance->status }}</span>
                            @else
                                <span class="badge bg-success p-2">{{ $attendance->status }}</span>
                            @endif
                        @else
                            <a href="{{ route('attendances.create', ['id' => $student->id]) }}" class="btn btn-warning btn-sm m-2">Status</a>
                        @endif
                         --}}
                        <a href="{{route('assignments.show',$assignment->id)}}" class="btn btn-info btn-sm m-2">View</a>
                        <a href="{{route('assignments.edit',$assignment->id)}}" class="btn btn-primary btn-sm m-2">Edit</a>
                        <form action="{{route('assignments.destroy',$assignment->id)}}" method="post" onclick="return confirm('Are you sure you want to delete this data?')" class="d-inline">
                            @csrf
                            @method('DELETE')
